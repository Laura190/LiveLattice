...
def process_image(image, source, target, psf_path, wavelength, camera_id, save_mip, overwrite_file, use_dask, num_decon_it, skewed_decon, decon_zsection_size, do_operations):
    ...
    if skewed_decon:
        ...
        if do_decon:
            ...
            for ind_z in range(num_zsections):
                ...
                with TemporaryOTF(psf_nobg_path, otf_bgrd=0, skewed_decon=True) as otf: # deconv parameters
                    with RLContext(section.shape, otf.path, dzdata, dxdata, dzpsf, dxpsf, deskew, rotate, width, skewed_decon=True) as ctx:
                        section_deconv = rl_decon(section, background=0, n_iters=num_decon_it, skewed_decon=True)

                        # to avoid boundary effect due to chuncking in all x,y,z, we divide only z in chuncks in case of large dataset
                        end_z = min(image.shape[0]-start_z, pad_size+decon_zsection_size)
                        if start_z == pad_size:
                            full_deconv = section_deconv[:end_z]
                        else:
                            full_deconv = np.concatenate([full_deconv, section_deconv[pad_size:end_z]])
        ...
        if do_deskewrotate:
            ...
            if use_dask:
                ...
                # Step 1
                swapped = np.swapaxes(processing, 0, 2)

                # Step 2
                scaled = scale_dask(swapped, dzdata, dxdata, angle)

                # Step 3
                sheared = shear_dask(scaled, angle)

                # move to CPU memory
                processing = sheared.map_blocks(cupy.asnumpy)
                ...
                # Step 2
                scaled = scale_cupy(swapped, dzdata, dxdata)

                # Step 3
                sheared = shear_cupy(scaled, angle)

                # move to CPU memory
                processing = cupy.asnumpy(sheared)
        ...
        if do_decon:
            ...
            for ind_z in range(num_zsections):
                ...
                with TemporaryOTF(psf_nobg_path, otf_bgrd=0, skewed_decon=False) as otf: # deconv parameters
                    with RLContext(section.shape, otf.path, dzdata, dxdata, dzpsf, dxpsf, deskew, rotate, width, skewed_decon=False) as ctx:
                        section_deconv = rl_decon(section, background=0, n_iters=num_decon_it, skewed_decon=False)
...
def scale_dask(img, dzdata, dxdata, angle):
    
    scale = np.eye(4)
    scale[0,0] = np.sin(angle*np.pi/180.0)
    scale[2,2] = dzdata / dxdata

    output_shape = get_output_dimensions(scale, img.shape)
    
    scaled_vol = affine_dask(img, np.linalg.inv(scale), 
                             output_shape=output_shape, output_chunks=img.chunksize)
    
#     print('\nSize of scaled volume:', scaled_vol.size*scaled_vol.itemsize/1E9, 'GB')
    return scaled_vol


def shear_dask(img, angle):
    
    img = np.flip(img, 2)
    shearfactor = np.cos(angle*np.pi/180.0) / np.sin(angle*np.pi/180.0)

    skew = np.eye(4)
    skew[2,0] = shearfactor
    
    output_shape = get_output_dimensions(skew, img.shape)
    
    sheared_vol = affine_dask(img, np.linalg.inv(skew), 
                              output_shape=output_shape, output_chunks=img.chunksize)
     
    # print('\nSize of sheared volume:', sheared_vol.size*sheared_vol.itemsize/1E9, 'GB')
    return sheared_vol


def scale_cupy(img, dzdata, dxdata):
    ...
    scaled_vol = affine_cupy(img, cupy.asarray(np.linalg.inv(scale)), 
...
def shear_cupy(img, angle):
    ...
    sheared_vol = affine_cupy(img, cupy.asarray(np.linalg.inv(skew)), 
                              output_shape=tuple(output_shape))
